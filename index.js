...
const bot = new Telegraf('8826683177:AAE5vQ2zMlFS84lTpcqzwhwblC1mz1Qupv8');

const GROUP_CHAT_ID = -1003895327942;
const MESSAGE_THREAD_ID = 170270;
...
// Fungsi Reset Harian Menyeluruh (Reset Counter, Antrean, dan Status)
async function checkDailyReset(KV = null) {
    const currentDate = new Date().toDateString();
    
    // Jika menggunakan KV Cloudflare, kita ambil tanggal terakhir dari KV
    let storedDate = lastResetDate;
    if (KV) {
        const kvDate = await KV.get('lastResetDate');
        if (kvDate) storedDate = kvDate;
    }

    if (currentDate !== storedDate) {
        orderCounter = 1;
        activeQueue = [];
        activeBartender = { id: null, name: null, lastActionTime: 0 };
        lastResetDate = currentDate;

        if (KV) {
            await KV.put('orderCounter', '1');
            await KV.put('activeQueue', JSON.stringify([]));
            await KV.put('activeBartender', JSON.stringify(activeBartender));
            await KV.put('lastResetDate', currentDate);
        }

        console.log('🔄 Hari baru terdeteksi: Semua data KV (Antrean, Tiket, & Bartender) di-reset bersih ke awal.');
    }
}
...
bot.command('jadibartender', async (ctx) => {
    await checkAndResetBartender(bot);
    ...
    activeBartender.lastActionTime = Date.now();

    await ctx.reply(`🎉 Selamat, **${userName}** sekarang resmi bertugas sebagai **Bartender**! 🍸\n_Catatan: Kamu memiliki batas waktu 5 menit per aksi untuk merespons pesanan._`, { parse_mode: 'Markdown' });
...
// ==========================================
// 2. FUNGSI VALIDASI TOPIC & PEMESANAN
// ==========================================
function isValidTopic(ctx) {
    return (
        ctx.chat &&
...
async function processOrder(botInstance, itemsList, userName, userUsername, targetInfo = "", ctxForReply = null) {
    await checkAndResetBartender(botInstance);
    ...
    if (activeBartender.id === null) {
        ...
        sentMsg = await botInstance.telegram.sendMessage(GROUP_CHAT_ID, messageContent, {
            parse_mode: 'Markdown',
            message_thread_id: MESSAGE_THREAD_ID
        });
    ...
    if (isFirstInQueue) {
        ...
        try {
            await botInstance.telegram.sendMessage(activeBartender.id,
...
barMenu.forEach(item => {
    ...
    bot.command(cmdName, async (ctx) => {
        ...
        await processOrder(bot, itemsList, userName, userUsername, "", ctx);
    });
});
...
bot.command('traktir', async (ctx) => {
    ...
    await processOrder(bot, itemsList, userName, userUsername, targetInfo, ctx);
});
...
barMenu.forEach(item => {
    bot.action(`order_btn_${item.id}`, async (ctx) => {
        ...
        await processOrder(bot, itemsList, userName, userUsername, "", null);
    });
});
...
bot.action(/start_(.+)/, async (ctx) => {
    await checkAndResetBartender(bot);
    if (activeBartender.id !== ctx.from.id) {
        return ctx.answerCbQuery('Kamu bukan bartender aktif!', { show_alert: true });
    }
    activeBartender.lastActionTime = Date.now();
    ...
    try {
        ...
        for (const item of order.items) {
            for (let q = 1; q <= item.qty; q++) {
                ...
                let step2 = `🛠️ [Gelas ${currentItemIndex}/${totalItemsCount}] ${item.steps[1]}...`;
...
bot.action(/note_custom_(.+)/, async (ctx) => {
    await checkAndResetBartender(bot);
    if (activeBartender.id !== ctx.from.id) return ctx.answerCbQuery('Bukan bartender aktif!', { show_alert: true });
    activeBartender.lastActionTime = Date.now();
...
bot.action(/note_skip_(.+)/, async (ctx) => {
    ...
    const order = activeQueue[orderIndex];
    await finalizeOrder(order, "Silakan dinikmati hidangannya!");
});
...
bot.on('text', async (ctx) => {
    ...
    if (state.action === 'finish_note') {
        ...
        await finalizeOrder(order, note);
...
async function finalizeOrder(order, note) {
    ...
    await bot.telegram.sendMessage(GROUP_CHAT_ID, finalMessage, {
        message_thread_id: MESSAGE_THREAD_ID
    });

    activeQueue.shift();
    sendNextQueuePrompt(activeBartender.id);
}

async function sendNextQueuePrompt(userId) {
    if (activeQueue.length > 0) {
        const nextOrder = activeQueue[0];
        try {
            await bot.telegram.sendMessage(userId,
...
// ==========================================
// ADAPTOR CLOUDFLARE WORKERS
// ==========================================
export default {
    async fetch(request, env, ctx) {
        // Cek reset harian setiap ada request masuk ke Worker
        await checkDailyReset();
        ...
        return new Response('Bar Master Bot is running on Cloudflare Workers!', { status: 200 });
    }
};
